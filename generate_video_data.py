...
def collect_content_data(base_path):
    ...
    content_dirs = {
        ...
        "exclusive_bts": "exclusive-bts-stills",
        "newsroom": "newsroom"
    }
...
def generate_js_rendering_logic():
    ...
    rendering_logic.append("                container.innerHTML = `<p style=\"text-align: center; color: var(--text-dim);\">No upcoming projects to display.</p>`;")
    rendering_logic.append("                return;")
    ...
    rendering_logic.append("                             `<img src=\"https://i.ytimg.com/vi/${item.media_id_or_url}/hqdefault.jpg\" alt=\"${item.title}\">` +")
    rendering_logic.append("                             `<div class=\"play-btn\" onclick=\"playVideo(this.parentNode, '${item.media_id_or_url}')\"><i class=\"fas fa-play\"></i></div>` +")
    rendering_logic.append("                             `</div>`;")
    ...
    rendering_logic.append("                <div class=\"feature-grid\">")
    rendering_logic.append("                    ${mediaHtml}")
    rendering_logic.append("                    <div class=\"feature-content\">")
    rendering_logic.append("                        <span class=\"feature-tag\">${item.tag}</span>")
    rendering_logic.append("                        <h2 class=\"luxury-font\">${item.title}</h2>")
    rendering_logic.append("                        <p class=\"feature-desc\">${item.description}</p>")
    rendering_logic.append("                        <a href=\"${learnMoreLink}\" target=\"_blank\" class=\"btn btn-primary\">Learn More</a>")
    rendering_logic.append("                    </div>")
    rendering_logic.append("                </div>")
    rendering_logic.append("            `;")
    rendering_logic.append("        }")
    ...
    rendering_logic.append("                container.innerHTML = `<p style=\"text-align: center; color: var(--text-dim);\">No BTS images to display.</p>`;")
    rendering_logic.append("                return;")
    rendering_logic.append("            }")
    rendering_logic.append("            container.innerHTML = exclusiveBtsData.map(item => `")
    rendering_logic.append("                <div class=\"bts-item\"><img src=\"${item.url}\" alt=\"${item.title}\"></div>")
    rendering_logic.append("            `).join('');")
    rendering_logic.append("        }")
    ...
    rendering_logic.append("                container.innerHTML = `<p style=\"text-align: center; color: var(--text-dim);\">No news articles to display.</p>`;")
    rendering_logic.append("                return;")
    rendering_logic.append("            }")
    rendering_logic.append("            container.innerHTML = newsroomData.map(article => `")
    rendering_logic.append("                <div class=\"news-card\">")
    rendering_logic.append("                    <span class=\"news-date\">${article.date}</span>")
    rendering_logic.append("                    <h3 class=\"luxury-font\">${article.title}</h3>")
    rendering_logic.append("                    <p style=\"color: var(--text-dim); font-size: 14px; margin-bottom: 20px;\">${article.description}</p>")
    rendering_logic.append("                    ${article.link ? `<a href=\"${article.link}\" target=\"_blank\" style=\"color: var(--emerald); text-decoration: none; font-size: 12px; font-weight: 700;\">READ MORE <i class=\"fas fa-arrow-right\"></i></a>` : ''}")
    rendering_logic.append("                </div>")
    rendering_logic.append("            `).join('');")
    rendering_logic.append("        }")
...
def update_index_html(html_filepath, new_js_data, new_js_rendering_logic):
    ...
    # 3. Update HTML structure for new sections
    if '<div id="latest-coming-soon-container"></div>' not in updated_html_content:
        updated_html_content = re.sub(
            ...
            r'\1<div id="latest-coming-soon-container"></div>',
    ...
    if '<div class="bts-slider reveal" id="bts-carousel">' not in updated_html_content:
        updated_html_content = re.sub(
            ...
            r'<div class="bts-slider reveal" id="bts-carousel">',
            updated_html_content, count=1
        )
    updated_html_content = re.sub(
    ...
    if '<div class="news-grid reveal" id="news-grid-container">' not in updated_html_content:
        updated_html_content = re.sub(
            ...
            r'<div class="news-grid reveal" id="news-grid-container">',
            updated_html_content, count=1
        )
    updated_html_content = re.sub(
